...
class VTUserCalendarViewController: UIViewController, JTCalendarDataSource, NSURLConnectionDelegate, NSURLConnectionDataDelegate {
    ...
    override func viewDidLoad() {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "showSelectedActivityDetails:", name: "userActivitySelected", object: nil)
    }
    ...
    func showSelectedActivityDetails(notification: NSNotification) {
        ...
        self.performSegueWithIdentifier("userActivityInfoSegue", sender: self)
    }
    ...
    func loadNewCalendarPageForActivities() {
        let components:NSDateComponents = NSCalendar.currentCalendar().components(.Weekday, fromDate: self.calendarContentView.currentDate!)
        ...
        if self.calendar.calendarAppearance.isWeekMode {
            ...
            let startDateOfCurrentShowingWeek:NSDate = self.calendarContentView.currentDate!.dateByAddingTimeInterval(NSTimeInterval(timeInterval))
            let endDateOfCurrentShowingWeek:NSDate = self.calendarContentView.currentDate!.dateByAddingTimeInterval(NSTimeInterval(60 * 60 * 24 * (7 - weekday + 1)))
            
            if startDateOfCurrentShowingWeek.compare(self.startDateOfCurrentBatchOfActivities!) == NSComparisonResult.OrderedAscending || endDateOfCurrentShowingWeek.compare(self.endDateOfCurrentBatchOfActivities!) == NSComparisonResult.OrderedDescending {   // current showing week in calendar contains dates that are outside the date range of current batch of activities, so we should send http request to server asking for activities data for a new date range
                self.getActivitiesForRecentMonths(self.calendar.currentDate!)
            }
    ...
    func connectionDidFinishLoading(connection: NSURLConnection) {
        ...
        let jsonArray = (try? NSJSONSerialization.JSONObjectWithData(self.responseData!, options: .MutableContainers)) as? [AnyObject]
    ...
    func getActivitiesForRecentMonths(date: NSDate!) {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "containerOfUserActivitiesForDateSegue" {
            let destinationViewController = segue.destinationViewController as! VTUserActivitiesForDateTableViewController
