...
class VTTeamActivitiesForDateTableViewController: UITableViewController, VTTeamCalendarViewDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (indexPath as NSIndexPath).row == 0 {   // if the cell is the first row of the section, add a separatorLine
            let separatorLineView:UIView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenSize.width, height: 1))
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        NotificationCenter.default.post(name: Notification.Name(rawValue: "activitySelected"), object: selectedActivity)
    }
